...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow verisight domains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*verisight\.io$/)) {
        return true;
      }
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@verisight.io" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@verisight.io" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
